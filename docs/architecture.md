...
| `list_skills` | — | List a workspace's skill catalog: `.tst/skills/*/SKILL.md`, with user-global `~/.tstdesk/skills/*/SKILL.md` winning a shared name; each tree falls back to its `.claude/skills` twin when it holds none of ours. Keyed like `list_commands` — the composer's `/` menu draws both listings (TD-4502). Acked with `skills`. |
...
| `skills` | connection | The skill catalog `list_skills` asked for (TD-4502). Same family as `commands`: connection-scoped, never pushed, never replayed — the menu re-asks on open. Distinct from the `skills_loaded` field on `instruction_stack`, which lists bodies already in the prompt. |
