...
def make_echo() -> tuple[ToolRegistry, ToolDispatcher]:
    ...
    async def echo_handler(session: object, message: str) -> str:
...
class TestApproveDeny:
    ...
    async def test_deny_returns_structured_message_to_model(self, tmp_path: Path) -> None:
        ...
        deltas = [e.delta for e in session.event_log.all_events if e.type == "assistant_delta"]
        assert "took another path" in "".join(deltas)  # mock streams per-word
