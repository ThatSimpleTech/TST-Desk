...
_WINDOWS_GRANDCHILD_PS1 = """
while (-not (Test-Path release.txt)) { Start-Sleep -Milliseconds 50 }
Set-Content kicked.txt x
"""


def _escape_probe(tmp_path: Path) -> str:
    """Platform escape probe. Workspace is *tmp_path* (the shell cwd)."""
    if sys.platform != "win32":
        return _ESCAPE_PROBE_CMD
    (tmp_path / "escape_probe.ps1").write_text(_WINDOWS_ESCAPE_PROBE_PS1, encoding="utf-8")
    (tmp_path / "escape_grandchild.ps1").write_text(_WINDOWS_GRANDCHILD_PS1, encoding="utf-8")
    return "powershell -NoProfile -File escape_probe.ps1"
...
async def _assert_group_gone(tmp_path: Path, seconds: float = 5.0) -> None:
    ...
    while not await asyncio.to_thread(pgid_file.exists):
        if asyncio.get_running_loop().time() > deadline:
            return
        await asyncio.sleep(0.02)
    raw = await asyncio.to_thread(pgid_file.read_text)
    pids = [int(token) for token in raw.split() if token.strip()]
    deadline = asyncio.get_running_loop().time() + seconds
    while True:
        ...
        if asyncio.get_running_loop().time() > deadline:
            pytest.fail(
                f"process group {pids} survived the reported kill — still alive: {survivors}"
            )
        await asyncio.sleep(0.02)


def _pid_alive(pid: int) -> bool:
    """True when *pid* (POSIX pgid / Windows PID) still exists."""
    if sys.platform == "win32":
...
def _windows_pid_alive(pid: int) -> bool:
    """OpenProcess + STILL_ACTIVE. Import-safe off Windows."""
    import ctypes
...
class TestTimeoutAndGroupKill:
    async def test_timeout_kills_process_group(self, tmp_path: Path) -> None:
        ...
        result = await dispatcher.dispatch(
            "c1", "shell", {"command": _escape_probe(tmp_path), "timeout_secs": 1}, session
        )
