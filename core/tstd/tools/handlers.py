...
def register_builtin_handlers(
    ...
    when set (TD-605); ``None`` leaves it unrestricted.  The write
    handler arrives with its story (TD-604); until then that tool returns
    the dispatcher's "no handler" error.
    """
