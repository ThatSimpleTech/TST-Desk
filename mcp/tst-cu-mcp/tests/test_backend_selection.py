...
class TestSelection:
    ...
    def test_linux_selected_for_linux(self) -> None:
        assert isinstance(get_backend("linux"), LinuxBackend)
...
class TestBackendName:
    ...
    def test_name_for_unsupported_platform_is_none_not_an_error(self) -> None:
        ...
        assert backend_name("freebsd") is None

    def test_name_for_supported_platforms(self) -> None:
        assert backend_name("darwin") == "darwin"
        assert backend_name("win32") == "windows"
        assert backend_name("linux") == "linux"


class TestProtocolConformance:
    @pytest.mark.parametrize("backend", [DarwinBackend(), WindowsBackend(), LinuxBackend()])
